...
const clamp=(v,a,b)=>Math.max(a,Math.min(b,v)),colors=['#a6f5cd','#bdb1ff','#ffcf92'],types=['flee','melee','ice'];
let player,ghosts,particles,time=0,last=0,flashTimer=0,cooldown=0,vac=false,keys={},joy={x:0,y:0},caught=0,active=true,sound=false,ac,noticeTime=0,lockedGhost=null,scare=0,battery=null,tableUsed=false,iceBolt=null;
...
function reset(){
lockedGhost=null;player={x:340*ROOM_SCALE,y:526*ROOM_SCALE,a:-Math.PI/2,hp:100,hurt:0,slow:0};particles=[];scare=0;time=0;caught=0;active=true;vac=false;keys={};joy={x:0,y:0};flashTimer=0;cooldown=0;battery=null;tableUsed=false;iceBolt=null;
ghosts=colors.map((color,i)=>({x:[210,505,500][i]*ROOM_SCALE,y:[215,228,465][i]*ROOM_SCALE,state:'warning',color,type:types[i],hp:100,stun:0,caught:false,seed:i*2.3,noSuction:0,touching:false,attackTime:0,locked:false,reveal:0,fireCd:1.2}));
$('#win').hidden=true;$('#knob').style.transform='';$('#vacuum').classList.remove('active');$('#fill').style.width='0%';$('#label').textContent='AIM → FLASH TO LOCK → VACUUM';say('Ghosts stay hidden until your light finds them. Bump the green-marked table for a battery.',5);ui();
}
...
if(!point)return;burst(g.x,g.y,g.color,15);if(lockedGhost===g)lockedGhost=null;Object.assign(g,point,{hp:100,stun:0,state:'warning',noSuction:0,touching:false,attackTime:0,locked:false,reveal:0,fireCd:1.2});burst(g.x,g.y,g.color,15);
}
...
function update(dt){time+=dt;scare=Math.max(0,scare-dt);cooldown=Math.max(0,cooldown-dt);flashTimer=Math.max(0,flashTimer-dt);noticeTime-=dt;player.hurt=Math.max(0,player.hurt-dt);player.slow=Math.max(0,player.slow-dt);if(!active)return;
const slowMul=player.slow>0?.55:1;
let mx=joy.x+(keys.d||keys.ArrowRight?1:0)-(keys.a||keys.ArrowLeft?1:0),my=joy.y+(keys.s||keys.ArrowDown?1:0)-(keys.w||keys.ArrowUp?1:0);const mag=Math.hypot(mx,my);if(mag>1){mx/=mag;my/=mag}if(!lockValid())lockedGhost=null;if(mag>.12&&!lockedGhost)player.a=Math.atan2(my,mx);move(mx*dt*(vac?118:185)*slowMul,my*dt*(vac?118:185)*slowMul);if(lockValid())player.a=Math.atan2(lockedGhost.y-player.y,lockedGhost.x-player.x);else lockedGhost=null;
...
if(h.state==='warning'){if(inBeam(h)){h.state='roam';h.attackTime=0;burst(h.x,h.y,h.color,10);tone(140,.2);say('It saw your light — now it’s hunting you!',2)}continue}
...
if(h.type==='flee'){
if(d<260){h.x-=dx/d*dt*85;h.y-=dy/d*dt*85}
}else if(h.type==='ice'){
h.fireCd-=dt;
if(d>380){h.x+=dx/d*dt*70;h.y+=dy/d*dt*70}else if(d<260){h.x-=dx/d*dt*55;h.y-=dy/d*dt*55}
if(!iceBolt&&h.fireCd<=0&&d<650){iceBolt={x:h.x,y:h.y,vx:dx/d*260,vy:dy/d*260,life:2.2};h.fireCd=2.6;tone(500,.15)}
}else if(h.state==='lunge'){h.x+=h.vx*dt;h.y+=h.vy*dt;h.rush-=dt;if(h.rush<=0){h.state='roam';h.attackTime=.65}}
else if(d<390&&h.attackTime<=0){h.state='lunge';h.rush=.6;h.vx=dx/d*200;h.vy=dy/d*200;tone(110,.12)}
else{h.x+=dx/d*dt*72;h.y+=dy/d*dt*72}
...
if(noticeTime<=0)$('#message').textContent=linked?'Keep holding VACUUM. Don’t let it escape!':current?.stun>0?'Hold VACUUM — ghosts relocate after 2 seconds without suction.':'Hidden ghosts wake when your light finds them. FLASH an awake ghost to stun, then hold VACUUM.';
...
function render(dt){particles=particles.filter(p=>p.life>0);particles.forEach(p=>{p.life-=dt;p.x+=p.vx*dt;p.y+=p.vy*dt});world3d.render({player,ghosts,particles,time,flashTimer,vac,lockedGhost,scare,battery,tableUsed,iceBolt})}
