...
let player,ghosts,particles,time=0,last=0,lightOn=false,vac=false,keys={},joy={x:0,y:0},caught=0,active=true,sound=false,ac,noticeTime=0,lockedGhost=null,scare=0,battery=null,tableUsed=false,iceBolt=null;
...
function reset(){
lockedGhost=null;player={x:340*ROOM_SCALE,y:526*ROOM_SCALE,a:-Math.PI/2,hp:100,hurt:0,slow:0};particles=[];scare=0;time=0;caught=0;active=true;vac=false;keys={};joy={x:0,y:0};lightOn=false;battery=null;tableUsed=false;iceBolt=null;
...
if(lit){if(h.stun<=0){burst(h.x,h.y,h.color,6);tone(500,.08);if(!lockedGhost||lockedGhost.caught||lockedGhost.state==='warning')lockedGhost=h}h.stun=Math.max(h.stun,1);h.locked=true;h.noSuction=0}
...
const current=target();$('#fill').style.width=current?(100-current.hp)+'%':'0%';$('#label').textContent=linked&&g&&!g.caught?'CAPTURING · '+Math.floor(100-g.hp)+'%':current?.stun>0?'LOCKED · VACUUM NOW · '+Math.max(0,2-current.noSuction).toFixed(1)+'s':'LIGHT ON TO SPOT · VACUUM TO CAPTURE';
if(noticeTime<=0)$('#message').textContent=linked?'Keep holding VACUUM. Don’t let it escape!':current?.stun>0?'Hold VACUUM — ghosts relocate after 2 seconds without suction.':lightOn?'Sweep the light around — it stuns whatever it touches.':'Turn the light on to see. It wakes hidden ghosts and stuns awake ones.';
}
...
function render(dt){particles=particles.filter(p=>p.life>0);particles.forEach(p=>{p.life-=dt;p.x+=p.vx*dt;p.y+=p.vy*dt});world3d.render({player,ghosts,particles,time,lightOn,vac,lockedGhost,scare,battery,tableUsed,iceBolt})}
