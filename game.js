...
function lockValid(){return lockedGhost&&!lockedGhost.caught&&lockedGhost.stun>0&&lockedGhost.state==='roam'&&Math.hypot(lockedGhost.x-player.x,lockedGhost.y-player.y)<380}
...
if(!point)return;burst(g.x,g.y,g.color,15);if(lockedGhost===g)lockedGhost=null;Object.assign(g,point,{hp:100,stun:0,state:'warning',noSuction:0,touching:false,attackTime:0,locked:false,reveal:0,fireCd:1.2,struggleCd:.9});burst(g.x,g.y,g.color,15);
}
...
const g=target();const linked=!!(g&&vac&&g.stun>0&&inBeam(g,340,.5));
