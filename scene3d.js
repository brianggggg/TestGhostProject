...
const MAX_PARTICLES=220;const sparksGeo=new THREE.BufferGeometry();const sparksPos=new Float32Array(MAX_PARTICLES*3);sparksGeo.setAttribute('position',new THREE.BufferAttribute(sparksPos,3));sparksGeo.setDrawRange(0,0);const sparks=new THREE.Points(sparksGeo,new THREE.PointsMaterial({color:'#c8fbe0',size:.07,transparent:true,opacity:.8}));scene.add(sparks);
let previous=new THREE.Vector3(),first=true;
return {render({player,ghosts:states,particles,time,flashTimer,vac,lockedGhost,scare,battery,tableUsed,iceBolt}){const x=px(player.x),z=px(player.y);robot.position.set(x,0,z);body.rotation.y=Math.PI/2-player.a;if(!first){roller.rotation.x+=(z-previous.z)/.32;roller.rotation.z-=(x-previous.x)/.32}previous.set(x,0,z);first=false;
...
ghostGlow[i].opacity=.12+.74*revealed;ghostGlow[i].emissiveIntensity=.1+.55*revealed;ghostEyeMat[i].opacity=Math.max(0,revealed*1.5-.5);ghostLight[i].intensity=.15+1.85*revealed;
...
let pcount=0;for(let k=0;k<particles.length&&pcount<MAX_PARTICLES;k++){const p=particles[k];if(p.life<=0)continue;const o3=pcount*3;sparksPos[o3]=px(p.x);sparksPos[o3+1]=.8+p.life*.5;sparksPos[o3+2]=px(p.y);pcount++}sparksGeo.attributes.position.needsUpdate=true;sparksGeo.setDrawRange(0,pcount);
